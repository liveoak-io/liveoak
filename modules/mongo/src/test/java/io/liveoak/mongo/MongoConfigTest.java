...
public class MongoConfigTest extends BaseMongoDBTest {
    ...
    public void testReadConfigOnRoot() throws Exception {
        ...
        ResourceState result = connector.read(new RequestContext.Builder().build(), BASEPATH + ";config");
    ...
    public void testReadConfigOnNonExistantRoot() throws Exception {
        ...
        try {
            connector.read(new RequestContext.Builder().build(), "foo;config");
            fail();
    ...
    public void testReadConfigOnCollection() throws Exception {
        ...
        try {
            connector.read(new RequestContext.Builder().build(), BASEPATH + "/" + methodName + ";config");
            fail();
    ...
    public void testReadConfigOnNonExistantCollection() throws Exception {
        ...
        // currently we don't have config options on collections, so this should fail
        try {
            connector.read(new RequestContext.Builder().build(), BASEPATH + "/foo;config");
            fail();
        } catch (ResourceNotFoundException e) {
            // expected
        }
    ...
    public void testModifyMongoConfig() throws Exception {
        ...
        ResourceState configResult = connector.read(new RequestContext.Builder().build(), BASEPATH + ";config");
        ...
        ResourceState messageResult = connector.read(new RequestContext.Builder().build(), BASEPATH + "/" + methodName + "/message");
        assertThat(messageResult.getProperty("text")).isEqualTo("hello world");

        // update the configuration values
        configResult.putProperty("db", "testB");
        ResourceState updateResult = connector.update(new RequestContext.Builder().build(), BASEPATH + ";config", configResult);
        assertThat(updateResult.getProperty("db")).isEqualTo("testB");

        // verify the new configuration values
        ResourceState configResultUpdated = connector.read(new RequestContext.Builder().build(), BASEPATH + ";config");
        ...
        ResourceState messageResultB = connector.read(new RequestContext.Builder().build(), BASEPATH + "/" + methodName + "/message");
    ...
    public void testModifyMongoInvalidDatabase() throws Exception {
        ...
        try {
            connector.update(new RequestContext.Builder().build(), BASEPATH + ";config", configResult);
            fail();
        } catch (ExecutionException e) {
            assertThat(e.getCause()).isInstanceOf(InitializationException.class);
        }
    ...
    public void testModifyMongoInvalidPort() throws Exception {
        ...
        ResourceState state = connector.update(new RequestContext.Builder().build(), BASEPATH + ";config", configResult);
