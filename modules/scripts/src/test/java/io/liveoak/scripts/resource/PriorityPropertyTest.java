...
public class PriorityPropertyTest extends BaseResourceTriggeredTestCase {
    ...
    public void priorityProperty() throws Exception {
        ...
        assertThat(execPost(RESOURCE_SCRIPT_PATH, "{ \"id\": \"default\", \"target-path\": \"/testApp\" }")).hasStatus(201);
        assertThat(toJSON(httpResponse.getEntity()).get("priority").asInt()).isEqualTo(5);

        // Test #2 - negative priority fails
        assertThat(execPost(RESOURCE_SCRIPT_PATH, "{ \"id\": \"negative\", \"target-path\": \"/testApp\", \"priority\": -1 }")).hasStatus(406);
        assertThat(httpResponse).isNotAcceptable();

        // Test #3 - zero priority fails
        assertThat(execPost(RESOURCE_SCRIPT_PATH, "{ \"id\": \"zero\", \"target-path\": \"/testApp\", \"priority\": 0 }")).hasStatus(406);
        assertThat(httpResponse).isNotAcceptable();

        // Test #4 - > 10 priority fails
        assertThat(execPost(RESOURCE_SCRIPT_PATH, "{ \"id\": \"toolarge\", \"target-path\": \"/testApp\", \"priority\": 11 }")).hasStatus(406);
        assertThat(httpResponse).isNotAcceptable();

        // Test #5 - valid priority
        assertThat(execPost(RESOURCE_SCRIPT_PATH, "{ \"id\": \"valid\", \"target-path\": \"/testApp\", \"priority\": 7 }")).hasStatus(201);
        ...
        assertThat(execPost(RESOURCE_SCRIPT_PATH, "{ \"id\": \"invalidupdate\", \"target-path\": \"/testApp\", \"priority\": 2 }")).hasStatus(201);
        ...
        assertThat(execPost(RESOURCE_SCRIPT_PATH, "{ \"id\": \"validtodefault\", \"target-path\": \"/testApp\", \"priority\": 2 }")).hasStatus(201);
        assertThat(toJSON(httpResponse.getEntity()).get("priority").asInt()).isEqualTo(2);
        assertThat(execPut(RESOURCE_SCRIPT_PATH + "/validtodefault", "{ \"target-path\": \"/testApp\", \"priority\": null }")).hasStatus(200);
        assertThat(toJSON(httpResponse.getEntity()).get("priority").asInt()).isEqualTo(5);
    }
}
